...
double Oscillator::ApplySmoothing(double value) {
    ...
    smoothed_value = smoothed_value * (1.0 - smoothing_factor_) + value * smoothing_factor_;
...
void Oscillator::Detach() {
    ...
    ESP_ERROR_CHECK(ledc_stop((ledc_mode_t)ledc_speed_mode_, (ledc_channel_t)ledc_channel_, 0));
...
void Oscillator::Write(int position) {
    ...
    ESP_ERROR_CHECK(ledc_set_duty((ledc_mode_t)ledc_speed_mode_, (ledc_channel_t)ledc_channel_, duty));
    ESP_ERROR_CHECK(ledc_update_duty((ledc_mode_t)ledc_speed_mode_, (ledc_channel_t)ledc_channel_));
}
