...
void Otto::Execute(int amplitude[SERVO_COUNT], int offset[SERVO_COUNT], int period,
    ...
    // v6优化：向上取整确保每步都走完，避免停在半步位置
    int cycles = (int)ceil(steps);
    ...
    // v6优化：动作结束后让腿脚归位到中立站姿（90°+offset）
    // 避免停在摆动的中间位置
    ...
    MoveServos(200, final_positions);  // 200ms平滑归位
}
...
void Otto::Walk(float steps, int period, int dir, int amount) {
    ...
    int A[SERVO_COUNT] = {25, 25, 25, 25, 0, 0};  // v5优化：降低振幅实现小步快走
...
void Otto::ShakeLeg(int steps, int period, int dir) {
    ...
    period = period - T2;
    period = (period > 200 * numberLegMoves) ? period : (200 * numberLegMoves);
