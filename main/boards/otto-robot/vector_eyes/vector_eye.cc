...
void VectorEye::SetMirrored(bool mirrored) {
...
void VectorEye::ApplyEffects() {
    ...
    if (blink_factor_ > 0.0f) {
        ...
        // 眨眼时眼睛向下移动一点
        current_config_.offset_y += static_cast<int16_t>(
            base_config_.height * blink_factor_ * 0.3f
        );
    }

    // 应用视线偏移
    int16_t max_look_offset = 15;  // 最大偏移像素
    
    // 镜像时 X 方向反转
    float effective_look_x = is_mirrored_ ? -look_x_ : look_x_;
    
    current_config_.offset_x += static_cast<int16_t>(effective_look_x * max_look_offset);
    current_config_.offset_y += static_cast<int16_t>(look_y_ * max_look_offset);
}
