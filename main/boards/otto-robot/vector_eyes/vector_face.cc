...
VectorFace::VectorFace(uint16_t screen_width, uint16_t screen_height, uint16_t eye_size)
...
void VectorFace::Draw() {
    ...
    // 绘制左眼
    EyeDrawer::Draw(
        left_eye_.GetCenterX(),
        left_eye_.GetCenterY(),
        left_eye_.GetCurrentConfig()
    );

    // 绘制右眼
    EyeDrawer::Draw(
        right_eye_.GetCenterX(),
        right_eye_.GetCenterY(),
...
void VectorFace::UpdateEyePositions() {
    ...
    right_eye_.SetCenter(center_x + eye_size_ / 2 + eye_inter_distance_, center_y);
}
