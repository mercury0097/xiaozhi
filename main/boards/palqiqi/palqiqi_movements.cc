...
void Otto::Bend(int steps, int period, int dir) {
  ...
  // 向左弯腰：左脚向外支撑(<90)，右脚向内收(<90)准备
  int bend1[SERVO_COUNT] = {
      90, 90, 65, 45, HAND_HOME_POSITION, HAND_HOME_POSITION};
  ...
  // 向右弯腰：左脚向内收准备，右脚向外支撑
  if (dir == -1) {
    bend1[2] = 135;  // 左脚向内收准备（对称45°）
    bend1[3] = 115;  // 右脚向外支撑（对称65°）
    bend2[2] = 35;   // 左脚弯腰位置
    bend2[3] = 118;  // 右脚保持支撑（对称62°）
  }

  // 放慢速度，使用贝塞尔曲线平滑过渡
  // 每个阶段时间延长，更稳定更平滑
  int prepare_time = 1500;  // 准备姿势时间
  int bend_time = 2000;     // 弯腰时间
  int return_time = 1500;   // 归位时间

  // Bend movement with bezier easing
  for (int i = 0; i < steps; i++) {
    // 准备姿势：使用 EASE_IN_OUT 平滑启动和停止
    MoveServosWithEase(prepare_time, bend1, EASE_IN_OUT);
    // 弯腰：使用 EASE_IN_OUT 保持平滑
    MoveServosWithEase(bend_time, bend2, EASE_IN_OUT);
    // 保持弯腰姿势
    vTaskDelay(pdMS_TO_TICKS(period * 0.6));
    // 回到站立：使用 EASE_IN_OUT 平滑归位
    MoveServosWithEase(return_time, homes, EASE_IN_OUT);
  }
}
...
void Otto::ShakeLeg(int steps, int period, int dir) {
  ...
  // 左脚方向与右脚相反：右脚50°准备 -> 左脚130°准备
  if (dir == 1) {
    shake_leg1[2] = 130;        // 左脚抬起准备（对应右脚50°）
    shake_leg1[3] = 110;        // 右脚支撑
    shake_leg2[2] = 70;         // 左脚抬高（对应右脚110°）
    shake_leg2[3] = 110;        // 右脚保持支撑
    shake_leg3[2] = 100;        // 左脚抖动位置（对应右脚80°）
